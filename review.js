...
const topics = {
  event: {
    ...
    content: [
      { type: 'info', text: '══ 核心模型 ══' },
      { type: 'pass', text: 'JS 是单线程的，但通过事件循环实现异步非阻塞' },
      ...
      { type: 'info', text: '── execution order (背到滚瓜烂熟) ──' },
      { type: 'log',  text: '1. sync code (call stack)' },
      { type: 'log',  text: '2. drain ALL microtasks (Promise.then, await, queueMicrotask)' },
      { type: 'log',  text: '3. pick ONE macrotask (setTimeout, setInterval, I/O, UI render)' },
      { type: 'log',  text: '4. goto step 2' },
      { type: 'info', text: '── 微任务 vs 宏任务 速查 ──' },
      { type: 'pass', text: 'micro: Promise.then/catch/finally, await, queueMicrotask, MutationObserver' },
      { type: 'pass', text: 'macro: setTimeout, setInterval, setImmediate(Node), I/O, requestAnimationFrame' },
      { type: 'info', text: '── quiz 1: 基础 ──' },
      ...
      { type: 'log',  text: '1,4 同步 → 3 微任务 → 2 宏任务' },
      { type: 'info', text: '── quiz 2: async/await ──' },
      { type: 'code', text: 'async function foo() {' },
      { type: 'code', text: '  console.log(1)' },
      { type: 'code', text: '  await Promise.resolve()' },
      { type: 'code', text: '  console.log(2)        // 等价于 .then(() => log(2))' },
      { type: 'code', text: '}' },
      { type: 'code', text: 'foo()' },
      { type: 'code', text: 'console.log(3)' },
      { type: 'pass', text: 'answer: 1 3 2' },
      { type: 'log',  text: 'foo() 同步执行到 await → 让出 → 3 同步 → 2 微任务' },
      { type: 'info', text: '── quiz 3: 嵌套 Promise ──' },
      ...
      { type: 'log',  text: '内层 .then(2) 在当前微任务轮次注册，优先于外层链式 .then(3)' },
      { type: 'info', text: '── Node.js 差异 ──' },
      ...
      { type: 'info', text: '── 面试话术 ──' },
      { type: 'log',  text: '"事件循环是 JS 实现异步的核心机制。每轮循环先执行同步代码，' },
      { type: 'log',  text: ' 然后清空所有微任务，再取一个宏任务执行。微任务包括 Promise.then' },
      { type: 'log',  text: ' 和 await，宏任务包括 setTimeout 和 I/O。"' },
    ]
  },
  ...
  call: {
    ...
    content: [
      { type: 'info', text: '── 原理 ──' },
      { type: 'pass', text: 'JS 的 this 取决于"谁调用函数"：obj.fn() → this === obj' },
      { type: 'pass', text: 'call/apply/bind 的本质：把函数挂为 context 的临时方法来调用' },
      { type: 'info', text: '══ call（立即调用，参数逐个传）══' },
      { type: 'log',  text: 'spec: fn.call(ctx, a, b) → 以 ctx 为 this 执行 fn' },
      ...
      { type: 'code', text: '}' },
      { type: 'warn', text: 'Symbol("tempFn"): 不用普通字符串 key，防止覆盖 ctx 已有属性' },
      { type: 'warn', text: 'Object(ctx): 原始类型(数字/字符串)需要包装成对象才能挂属性' },
      { type: 'info', text: '── real-world ──' },
      { type: 'log',  text: 'Object.prototype.toString.call([]) → "[object Array]" (类型检测)' },
      { type: 'log',  text: 'Array.prototype.slice.call(arguments) → 类数组转真数组' },
      { type: 'info', text: '══ apply（参数是数组，其余同 call）══' },
      ...
      { type: 'code', text: '  return result' },
      { type: 'code', text: '}' },
      { type: 'warn', text: 'ES6+: Math.max(...arr) 已替代 apply，apply 主要出现在面试中' },
      { type: 'info', text: '══ bind（返回新函数，不立即调用）══' },
      { type: 'log',  text: 'spec: fn.bind(ctx, a) → 返回新函数 boundFn，调用时 this === ctx' },
      { type: 'pass', text: 'key diff: call/apply 立即执行，bind 返回新函数（闭包 + 延迟执行）' },
      ...
      { type: 'code', text: '}' },
      { type: 'warn', text: 'const originalFn = this: 必须存原函数，内层 this 已经变了' },
      { type: 'warn', text: 'advanced: if called with new, this should be new instance not ctx' },
      { type: 'info', text: '── real-world ──' },
      { type: 'log',  text: 'setInterval(this.tick.bind(this), 1000) — 回调中锁定 this' },
      { type: 'log',  text: 'log.bind(null, "WARN", "DB") — partial application / 预填参数' },
      { type: 'log',  text: 'React class: this.handleClick = this.handleClick.bind(this)' },
      { type: 'pass', text: 'ES6 箭头函数自动捕获词法 this — 很多场景已替代 bind' },
      { type: 'info', text: '── 速记对比 ──' },
      { type: 'pass', text: 'call:  fn.call(ctx, a, b)       立即执行，参数逐个' },
      { type: 'pass', text: 'apply: fn.apply(ctx, [a, b])    立即执行，参数数组' },
      { type: 'pass', text: 'bind:  fn.bind(ctx, a)(b)       返回新函数，支持偏应用' },
      { type: 'pass', text: '共同点: Symbol 临时属性 → 方法调用改 this → delete 清理' },
    ]
  },
