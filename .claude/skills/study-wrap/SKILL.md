...
🚀 你来跑（同步到 GitHub）：
   cd ~/preview && ./sync.sh out "Day N 完成 xxx，明天 Day M"

晚安，明天 /study-context 接着干。
```

## 步骤 5：等用户同步，**不要主动 git push**

涉及共享状态的命令（git push）默认让用户自己跑。
如果用户回复"帮我跑 sync"或"你来"，再用 Bash 执行。

## ❌ 不要做的事

- 不要写超过 10 行的总结
- 不要在收工时教育 / 批评用户（即使没完成里程碑）
- 不要主动 git commit / push（除非用户授权）
